CREATE TABLE IF NOT EXISTS `data-etl-to-bigquery.ctm_data.activities_raw_daily_lookback` AS
SELECT
  raw.*,
  TIMESTAMP(NULL) AS ctm_sync_loaded_at,
  CAST(NULL AS STRING) AS ctm_sync_run_id,
  CAST(NULL AS STRING) AS ctm_sync_window_start,
  CAST(NULL AS STRING) AS ctm_sync_window_end,
  CAST(NULL AS INT64) AS ctm_sync_lookback_days
FROM `data-etl-to-bigquery.ctm_data.activities_raw_daily` AS raw
WHERE FALSE;

ALTER TABLE `data-etl-to-bigquery.ctm_data.activities_raw_daily_lookback`
ADD COLUMN IF NOT EXISTS ctm_sync_loaded_at TIMESTAMP;
ALTER TABLE `data-etl-to-bigquery.ctm_data.activities_raw_daily_lookback`
ADD COLUMN IF NOT EXISTS ctm_sync_run_id STRING;
ALTER TABLE `data-etl-to-bigquery.ctm_data.activities_raw_daily_lookback`
ADD COLUMN IF NOT EXISTS ctm_sync_window_start STRING;
ALTER TABLE `data-etl-to-bigquery.ctm_data.activities_raw_daily_lookback`
ADD COLUMN IF NOT EXISTS ctm_sync_window_end STRING;
ALTER TABLE `data-etl-to-bigquery.ctm_data.activities_raw_daily_lookback`
ADD COLUMN IF NOT EXISTS ctm_sync_lookback_days INT64;

CREATE OR REPLACE VIEW `data-etl-to-bigquery.ctm_data.activities_raw_daily_lookback_merged` AS
WITH unioned AS (
  SELECT
    raw.*,
...
QUALIFY ROW_NUMBER() OVER (
  ...
  ORDER BY _candidate_priority DESC, _candidate_loaded_at DESC NULLS LAST
) = 1;

CREATE OR REPLACE VIEW `data-etl-to-bigquery.ctm_data.activities_combined_lookback_candidate` AS
SELECT
  batch.*,
...
SELECT
  ...
  is_spam,
  is_spam_raw
FROM `data-etl-to-bigquery.ctm_data.activities_raw_daily_lookback_merged`;

CREATE OR REPLACE VIEW `data-etl-to-bigquery.ctm_data.activities_data_lookback_candidate` AS
SELECT * FROM `data-etl-to-bigquery.ctm_data.activities_combined_lookback_candidate`;
