-- Rollback artifact: executable DDL restoring the console-only _90d view as captured 2026-07-17.
CREATE OR REPLACE VIEW `data-etl-to-bigquery.ctm_data.activities_combined_lookback_candidate_90d` AS
SELECT
  batch.*,
...
WHERE (DATE(batch.called_at) >= CURRENT_DATE - 90 OR batch.called_at IS NULL)
...
SELECT
  ...
  is_spam,
  is_spam_raw
FROM `data-etl-to-bigquery.ctm_data.activities_raw_daily_lookback_merged_90d`
